...
impl AppState {
    fn upsert_pending(&self, nostr_pubkey: String, handle: String) -> AccountLinkRecord {
    ...
    fn upsert_ready(&self, nostr_pubkey: String, handle: String, did: String) -> AccountLinkRecord {
        let mut links = self.links.lock().unwrap();
        let now = Utc::now();
        let record = links
            .entry(nostr_pubkey.clone())
    ...
    fn get_by_pubkey(&self, nostr_pubkey: &str) -> Option<AccountLinkRecord> {
        self.links.lock().unwrap().get(nostr_pubkey).cloned()
    }

    fn disable_by_pubkey(&self, nostr_pubkey: &str) -> Option<AccountLinkRecord> {
        let mut links = self.links.lock().unwrap();
    ...
    fn get_by_handle(&self, handle: &str) -> Option<AccountLinkRecord> {
        self.links
            .lock()
            .unwrap()
            .values()
...
pub fn app() -> Router {
    ...
    Router::new()
        ...
        .route("/api/account-links/provision", post(routes::provision::handler))
        .route("/api/account-links/:nostr_pubkey/status", get(routes::status::handler))
        .route("/api/account-links/:nostr_pubkey/disable", post(routes::disable::handler))
        .route("/api/account-links/:nostr_pubkey/export", get(routes::export::handler))
