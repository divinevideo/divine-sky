...
fn make_video_event(url: &str, sha256: &str) -> NostrEvent {
    make_signed_event(
        ...
        "contract test",
        vec![
            vec!["title".into(), "Contract".into()],
            vec!["url".into(), url.into()],
            vec!["x".into(), sha256.into()],
            vec!["d".into(), "custom-rkey-should-not-be-used".into()],
        ],
    )
}
...
async fn new_video_posts_use_create_record_with_validate_true_and_no_custom_rkey() {
    ...
    let blossom_path = "/blob/contract-video.mp4";
    ...
    match result {
        ...
        other => panic!("expected publish result, got {other:?}"),
    }
