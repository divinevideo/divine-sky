...
>
> **NEXT LIKELY WALL after PLC (audited, NOT yet fixed — needs the live PDS round-trip to confirm):** the bridge calls rsky-pds `com.atproto.server.createAccount` with only `{did, handle}` + `Basic admin:<password>`. Per the rsky reference (`rsky-pds/src/apis/com/atproto/server/create_account.rs`): (a) supplying a `did` requires admin auth (the `requester` check ~line 352) — confirm `Basic admin:` resolves to admin in this PDS's auth_verifier, else `AuthRequiredError`; (b) createAccount may require a `password` (~line 341 `InvalidPassword`) and/or `invite_code` (~line 276) unless treated as admin-import; (c) **the did-import path creates the account `deactivated = true`** — a deactivated repo won't serve records, so the bridge likely must call an activate step (e.g. `com.atproto.server.activateAccount`) before crossposts are visible. Verify these against the deployed PDS once provisioning reaches it; do NOT speculatively patch without the live round-trip (the path has never executed, so behavior must be observed, not guessed).
