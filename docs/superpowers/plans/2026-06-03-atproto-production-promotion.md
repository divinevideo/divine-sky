...
> **BLOCKED until the PLC fixes ship.** First attempt (2026-06-03) FAILED. Two compounding PLC bugs (both fixed on branch `atproto-production-rollout`, both TDD-covered):
> 1. `create_did` POSTed the genesis op to `/` instead of `/:did` (commit "fix(atbridge): POST PLC genesis op to /:did").
> 2. `derive_did_plc` hashed the UNSIGNED op; the spec/rsky hash the SIGNED op (commit "fix(atbridge): derive did:plc from the SIGNED genesis op"). These compound: PLC returns an empty body, so create_did posts to `POST /:derived_did` and returns that DID — a wrong derivation is both rejected by the directory and mislabels the account.
> **A staging image built from these commits must be deployed before Step 1 can succeed.** Until then opt-in lands in `provisioning_state=failed` with the (truncated) `provisioning_error='creating did:plc via PLC directory'` (see Chunk G for the error-visibility fix; real cause was the 404).
...
>
> **NEXT WALLS after PLC — audited deeper 2026-06-03 against the rsky reference (`rsky-pds/src/apis/com/atproto/server/`). Each is real; left UNFIXED on purpose because they need the live PDS round-trip and/or a maintainer's design intent, and guessing on a never-run path adds bugs.**
>
> **Wall 2 — createAccount auth/shape.** Bridge sends `{did, handle}` + `Basic admin:<password>`. rsky: supplying a `did` requires admin auth (`requester` check, create_account.rs:352 → else `AuthRequiredError`); may require `password` (~341) / `invite_code` (~276) unless admin-import. Confirm `Basic admin:` resolves to admin in this PDS's auth_verifier.
>
> **EVIDENCE UPDATE 2026-06-05 (from the live staging PDS, `com.atproto.sync.listRepos`):** the staging PDS holds **33 repos** with `*.staging.dvines.org` handles (e.g. `bridgefinal.staging.dvines.org`) — so the bridge HAS provisioned accounts historically (correcting the earlier "provisioning never succeeded" framing; what's true is the current bridge DB is empty AND the currently-deployed image `3ba324c` has the `/` PLC bug, so provisioning has been broken since it deployed). Decisively: **24 of the 33 repos are `active:false`** — direct, live confirmation that Wall 3 (created-deactivated, never-activated) is REAL and the common outcome, not just inferred. The 9 active repos contain only empty `smokeassoc…/smokeacl…` posts (manual PDS smoke-test stubs, `createdAt:None`) — i.e. **no genuine video crosspost has ever been produced**, even on the active repos. So: provisioning partially worked long ago, the activate step is missing (24 dead repos prove it), and the publish path has never produced a real video post.
>
> **Wall 3 — account created DEACTIVATED, bridge never activates (CONFIRMED bug — 24 inactive repos on the live PDS).** The did-import path sets `deactivated = true` (create_account.rs:361). The bridge calls `createAccount` and **never calls `activateAccount`** (grep of divine-atbridge/src = no matches) → the repo won't serve records → **crossposts won't appear on Bluesky even after provisioning "succeeds."** The fix is concrete and the pieces exist: `createAccount` returns `accessJwt`/`refreshJwt` for the new account (rsky-lexicon server.rs:143–152), and `activateAccount` needs `AccessFull` auth *as that DID* (activate_account.rs:16,22) — i.e. call `POST /xrpc/com.atproto.server.activateAccount` with `Authorization: Bearer <accessJwt>`. **Today `pds_accounts.rs::create_account` discards the response body (`Result<()>`)** — it must capture `accessJwt` and chain an activate call. TDD-able with a mock PDS (assert activate is called with the returned token). Hold until staging confirms the deactivated state empirically, then implement.
>
> **Wall 4 — per-account publish auth (OPEN architectural question, not yet answerable from source).** `publisher.rs` writes records (`com.atproto.repo.createRecord`) with a single shared `auth_token` Bearer (`PdsClient::new(base_url, auth_token)`), but ATProto repo writes must be authorized as the repo's own DID. Either that shared token is a service token rsky accepts for arbitrary DIDs (verify against the auth_verifier — cf. `memory: rsky-pds-video-upload-blocker`, which shows the verifier is picky about token types), OR each account needs its own session and the bridge must store/use the per-account `accessJwt`. This determines whether the activation `accessJwt` from Wall 3 also needs to be persisted for publishing. **Resolve with a maintainer or the live round-trip before building** — do not guess the auth model.
