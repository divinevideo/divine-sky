...
> **STATUS 2026-06-05:** Tasks 1–4 DONE on branch `feat/atbridge-rsky-native-provisioning`
> (commits: config; rsky-native provisioning; runbook). All 92 lib + integration tests
> green, fmt clean, no new clippy warnings, full DB-backed suite passes against local
> `divine_bridge`. Implementation deviations from the plan below: (a) no `PDS_INVITE_REQUIRED`
> config toggle — the bridge always mints a single-use invite (rsky admin can always do so;
> simpler and correct since invites are required everywhere); (b) the generated account
> password is NOT persisted (rsky admin can reset; bridge re-auths via stored session/refresh);
> (c) retry of a link that already carries a DID just re-marks ready instead of re-creating
> (avoids minting a duplicate DID — plan didn't specify this edge); (d) `create_pds_account_and_mark_ready`
> was removed (folded into create_new_link). **Task 5 (live staging deploy + e2e) is the only
> remaining step.**
...
> **Contract verified against rsky source 2026-06-05** (`rsky` checkout `7e5e78a`,
> `rsky-lexicon/src/com/atproto/server.rs:40` + `rsky-pds/.../create_account.rs:388-392`):
> - `CreateAccountInput.recovery_key` deserializes from JSON **`recoveryKey`** (camelCase) —
>   the bridge sends `recoveryKey`, so it is NOT silently dropped. ✓
> - `format_did_and_plc_op` builds `rotation_keys` with the user `recovery_key` pushed FIRST,
>   then the PDS rotation key — so the recovery key is highest priority. ✓
> - Re-confirm both against the DEPLOYED image (was `87151fc`, behind tip) during step 1.
>
> **BLOCKER (IaC, separate repo — handoff spec required):** the two new env vars
> `PLC_RECOVERY_ROTATION_DID_KEYS` and `ACCOUNT_EMAIL_DOMAIN` must be wired into the staging
> bridge ExternalSecret + overlay, or `recovery_keys` arrives EMPTY and the recovery key
> silently no-ops (account provisions fine, crosspost works, but the DID has no recovery key).
> `ACCOUNT_EMAIL_DOMAIN` defaults to `divine.video` in code so it's optional;
> `PLC_RECOVERY_ROTATION_DID_KEYS` is the make-or-break one.
>
> **PASS/FAIL GATE (not "video plays"):** after provisioning a fresh test account, fetch
> `https://plc.directory/<did>/data` and assert `rotationKeys[0]` == the staging recovery
> `did:key:zQ3shq8UUvhTbj8bzTyCnGkT6C7pFDVKAB23bd2JcsENaSgjk` and `rotationKeys[1]` == rsky's
> operational key. Video playback is necessary but NOT sufficient — it passes even when the
