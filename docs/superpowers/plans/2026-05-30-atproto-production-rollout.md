...
4. **A correction:** the smoke script had **no** `token_endpoint` assertion (only `authorization_endpoint` + `par`), so this plan's earlier Task A1 referenced a phantom assertion. **A1 is now done** (script fixed to `/api/atproto/oauth/*`). **A2 is nearly a no-op:** exactly one genuine ATProto reference was fixed (the auth-server-metadata test example in `2026-03-28-login-divine-video-atproto-boundary.md`, which asserts `scopes_supported: atproto` + `require_pushed_authorization_requests` — unambiguously the ATProto profile). The other remaining `/api/oauth/*` references are legitimate and were left alone: they describe keycast's *generic* Nostr/UCAN OAuth, including the explicitly-rejected "Option 1" in superseded design docs — sweeping those would corrupt accurate history.
